...
export async function sendInvoiceNotification(email: string, invoiceNumber: string, invoiceId: string) {
  ...
  return await sendEmail({
    ...
    html: `<p>Your landlord has generated invoice <strong>${safeInvoiceNumber}</strong>.</p><p><a href="${url}">View and download your invoice</a></p>`,
  })
}
...
export async function sendJoinRequestNotification(landlordEmail: string, tenantName: string) {
  ...
  return await sendEmail({
    ...
    html: `<p><strong>${safeTenantName}</strong> has submitted a request to join your building.</p><p><a href="${baseUrl()}/landlord/join-requests">Review join requests</a></p>`,
  })
}
...
export async function sendJoinRequestResult(email: string, approved: boolean, buildingName: string) {
  ...
  const body = approved
    ? `<p>Congratulations! Your request to join <strong>${safeBuildingName}</strong> has been approved.</p><p><a href="${baseUrl()}/tenant">Go to dashboard</a></p>`
    : `<p>Your request to join <strong>${safeBuildingName}</strong> was not approved at this time.</p>`
...
export async function sendPaymentProofNotification(landlordEmail: string, tenantName: string, invoiceNumber: string) {
  ...
  return await sendEmail({
    ...
    html: `<p><strong>${safeTenantName}</strong> has submitted payment proof for invoice <strong>${safeInvoiceNumber}</strong>.</p><p><a href="${baseUrl()}/landlord/invoices">Review payments</a></p>`,
  })
}
...
export async function sendPaymentVerificationResult(email: string, verified: boolean, invoiceNumber: string) {
  ...
  const body = verified
    ? `<p>Your payment for invoice <strong>${safeInvoiceNumber}</strong> has been verified. Thank you!</p>`
    : `<p>Your payment for invoice <strong>${safeInvoiceNumber}</strong> could not be verified. Please resubmit or contact your landlord.</p>`
  return await sendEmail({ to: email, subject, html: body })
}
...
export async function sendMaintenanceNotification(landlordEmail: string, title: string) {
  ...
  return await sendEmail({
    ...
    html: `<p>A new maintenance request has been submitted: <strong>${safeTitle}</strong></p><p><a href="${baseUrl()}/landlord/maintenance">Review requests</a></p>`,
  })
}
...
export async function sendMaintenanceStatusUpdate(email: string, title: string, status: string) {
  ...
  return await sendEmail({
    ...
    html: `<p>Your maintenance request <strong>${safeTitle}</strong> has been updated to: <strong>${safeStatus}</strong></p><p><a href="${baseUrl()}/tenant/maintenance">View your requests</a></p>`,
  })
}
